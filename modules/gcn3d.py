...
def pc_normalize(vertices):
    pc_mean = torch.mean(vertices, dim=1, keepdim=True)
    pc_direction = vertices - pc_mean
    scale_factor = torch.max(torch.sqrt(torch.sum(pc_direction ** 2, dim=-1, keepdim=True)), dim=-2, keepdim=True)[0]
    pc_norm = pc_direction / scale_factor
    return pc_norm
...
class Conv_layer(nn.Module):
    ...
    def forward(self, 
        ...
        # 根据点云的输入feature计算每一个kernel核中心点与三个support的feature
        ...
        # support的feature乘归一化的中心点与相邻点之间的方向向量与归一化的核的方向向量的余弦相似度
...
class Pool_layer(nn.Module):
    ...
    def forward(self, 
        ...
        return vertices_pool, feature_map_pool, sample_idx
...
def test():
    ...
    # 每次pool之后点云结构发生变化，都需要重新获取一次neighbor
